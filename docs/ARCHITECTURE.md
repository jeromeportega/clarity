...
- **Plaid bank sync** (`adapters/plaid/`): the launch path for bank data. The core speaks to a `PlaidClient` port (`plaid-client.ts`; the SDK is forbidden in core) and `sync.ts#syncPlaidItem` runs `accountsGet` → upsert `accounts` (`source = 'plaid'`, `plaid_item_id` + `external_id`), then walks `/transactions/sync` from the Item's stored cursor: `added` inserts (idempotent on the provider id; a posted transaction that names the pending one it replaces UPDATES that row in place, so our id and its matches survive), `modified` updates by provider id, `removed` deletes the row and its matches unless it was just replaced; the cursor is persisted with each page in the same transaction. Amounts flip sign (Plaid positive = money out; ours is negative cents) and merchants go through the bank adapter's cleaner. `apps/web/lib/plaid/` is the composition root: the SDK client (`PLAID_CLIENT_ID`/`PLAID_SECRET`/`PLAID_ENV`), access tokens encrypted at rest with AES-256-GCM under `PLAID_TOKEN_KEY` (`token-cipher.ts`), `syncHouseholdBanks`, and — sandbox only — `connectSandboxBank`, which creates Plaid's test Item without Link. Link (a real bank) is the next step.
...
| `plaid_items` | One Plaid Item (institution login) per row: encrypted `access_token`, `/transactions/sync` `cursor`, `status` (`ok` / `error` / `login_required`), last sync and error. | `household_id` FK |
| `accounts` | Bank/card accounts; `source` (`manual` / `file` / `plaid`), and for synced ones `plaid_item_id` + `external_id` (Plaid account id, unique per Item), `subtype`, `mask`. | `household_id` FK |
| `transactions` | Bank lines; `dedup_key` + `ux_transactions_dedup`; synced lines also carry `external_id` (unique per account), `pending`, `pending_external_id` and Plaid's `category_hint`. | via `account_id` |
...
- **Migrations:** `modules/finance/db/migrations/0000`–`0010`, generated by drizzle-kit (`npm run db:generate` / `db:migrate` in `modules/finance`). `createTestDb()` applies them to a throwaway temp file for every test.
...
- **Demo seed:** `npm run seed:demo` → `core/seed/demoHousehold.ts` inserts a household, one account, two orders, two receipts, four transactions, then runs `reconcile()` over a hand-written `ReconcileInputs` literal and persists through `DrizzleReconcileSink`.
...
| `POST /api/plaid/sync` | writer (session or `x-reconcile-token`) | Pull new bank activity for every connected Item in the writer's household, then reconcile. 503 when Plaid is not configured. |
| `POST /api/plaid/sandbox/connect` | writer (session or `x-reconcile-token`) | Sandbox only (404 otherwise): connect Plaid's test institution to the writer's household without Link and run the first sync. |
...
- **Database reads must never pass through a framework fetch cache.** `@libsql/client` reaches Turso over HTTP through the global `fetch`; Next.js patches that function, and on Vercel a fetch it deems cacheable lands in the persistent Data Cache — keyed by URL + body, shared across deployments, surviving `vercel --force`. In September 2026 the queue's "unmatched transactions" query (unchanging text and parameters) was answered from a response recorded weeks earlier: new bank lines never appeared in production while the same build against the same database was right locally. `createDb()` now builds every remote client with `uncachedFetch` (`cache: 'no-store'` on every request — `db/client.ts#clientConfig`), which bypasses cache reads as well as writes, so no purge is needed after deploy. The rule for contributors: never construct a `@libsql/client` yourself inside a Next.js request path; go through `createDb()`.
- **A receipt uploaded from a "Receipt wanted" row is not pinned to that charge.** `/receipts?txn=` only names the charge; the upload runs the ordinary matcher (±3 days, ±$15, name similarity), which may pick another bank line or none — in which case the offer stays and the receipt counts no dollars. Threading the id through the upload as a `ConfirmedMatch` is the next queue step. Also: `listUnmatchedTransactions` treats a transaction with only `rejected` match rows as matched, so rejecting the wrong receipt for a Costco charge also ends the offer to upload the right one.
- **Receipt images predating durable storage have no image.** Uploads now land in the private Blob store (or the local data directory) under `receipts/<household>/<imageHash>` and `/api/receipts/image/[receiptId]` serves them after the household check; receipts uploaded before this (whose bytes went to `/tmp`) and digital imports (no image at all) answer 404 there, and the evidence page still shows the link.
...
- **The evidence page cannot tell whether an image exists** before linking it; a digital receipt's link lands on a 404 with a plain message.
...
- **Synced bank lines that are still pending are stored but invisible** to the reconciler and the queue until they post (`transactions.pending`); when a posted line replaces its pending one the row is updated in place. A `modified` line whose amount or date changed withdraws any human `manual` match on it so it returns to the queue; a `removed` line deletes the row and its matches with no tombstone yet, and there is no way to disconnect a bank from the product yet (`/item/remove` and a delete route are the next Plaid step).
