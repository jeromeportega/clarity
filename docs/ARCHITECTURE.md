...
| `AI_GATEWAY_API_KEY` / Vercel OIDC | Live vision + SKU resolution go through the Vercel AI Gateway (`ai` with `provider/model` ids). On Vercel the deployment authenticates with its own OIDC token — nothing to set; locally an `AI_GATEWAY_API_KEY`, or the `VERCEL_OIDC_TOKEN` that `vercel env pull` writes. Without either, the upload route and the eval harness use recorded fixtures. `RECEIPT_AI=live|recorded` forces the choice; `CLARITY_VISION_MODEL` / `CLARITY_RESOLVER_MODEL` pick the models (`receipts/model-ids.ts`). |
