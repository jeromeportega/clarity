...
| `POST /api/ingest/orders` | writer (session or `x-reconcile-token`) | multipart `file` (25 MiB cap); imports into the demo household (`core/scope`), then reconciles. |
| `POST /api/ingest/costco` | writer (session or `x-reconcile-token`) | multipart `file` = saved `WarehouseReceiptDetail` JSON (25 MiB cap); receipts + line items into the demo household, then `learnFromDigitalReceipts` (response carries `dictionary`), then reconciles. |
| `POST /api/reconcile` | writer (session or `x-reconcile-token`) | JSON `{ householdId }` (must be the demo household today; 403 otherwise; 400 without a body, so a bare probe never starts a run); runs `reconcileHousehold` on demand and returns its summary. |
...
- **Writes** — `apps/web/app/lib/auth/writer.ts#requireWriter(req)` (routes) / `requireWriterFromAction()` (server actions): a signed-in person acts on their own household (API writes additionally require a same-origin `Origin`); otherwise a valid `x-reconcile-token` acts on the demo household — the operator's credential for imports, eval runs and scripts (the bank route further checks the named account belongs to the writer's household when the writer is a session). Neither, and the write is 401.

The session itself comes from **Clerk** (`@clerk/nextjs`): `app/lib/auth/session.ts#getPrincipal()` is the only file that consults it. It maps the Clerk user id to a household through `core/auth/membership.ts#resolveMembership` — a known member gets their household; a first sign-in provisions a household of their own and makes them its owner (tables `users`, `household_members`, migration 0007). Sign-in exists only when both `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` are set: `middleware.ts` mounts `clerkMiddleware()` and `layout.tsx` the `ClerkProvider` only then, and `getPrincipal` returns `null` otherwise, so tests, the public demo and a fresh clone run with no session anywhere. `/sign-in` and `/sign-up` render Clerk's components; `SiteNav` shows the user menu, a sign-in button, or the public-demo badge.
...
- **Tenancy is per person, not yet per table.** A signed-in person's household comes from `household_members`; the demo household is still a code constant (`DEMO_HOUSEHOLD_ID`) for the public demo and the token's writes. `matches` is scoped through `transactions → accounts`; `categories` is the shared taxonomy; `sku_dictionary` still carries no `household_id` (one dictionary for the deployment — the pooling-with-consent question); `review_decisions.household_id` has no FK. A person in several households gets the one they joined first; switching is not built. `/api/ingest/bank` still trusts a token caller's `accountId`.
