...
| `RECONCILE_MUTATION_TOKEN` | Yes | Secret that gates all mutation routes (`x-reconcile-token` header). Generate with `openssl rand -hex 32`. Read only by `apps/web/app/lib/auth/token.ts`; a test fails if any other file under `apps/web` mentions it |
