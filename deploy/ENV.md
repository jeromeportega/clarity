...
| `AI_GATEWAY_API_KEY` | Local / CI / other hosts | Vercel AI Gateway key for live vision + SKU resolution where there is no Vercel OIDC token (scripts, `npm run vision:eval`, CI, another host). Its presence turns live models on. Never needed on Vercel itself — see `RECEIPT_AI` |
| `VERCEL_OIDC_TOKEN` | Local dev (written by `vercel env pull`) | The 12-hour token `vercel env pull` writes to `.env.local`; the gateway accepts it in place of an API key, and its presence turns live models on. On a deployment the token arrives per request, not as a variable |
| `RECEIPT_AI` | Private deployment: `live` | `live` or `recorded` overrides the credential-based inference. A Vercel deployment has no credential variable, so it opts in with `live` and authenticates to the gateway with its OIDC token (needs the project's OIDC federation on, the default). Unset → previews and the public demo stay on recorded fixtures and spend nothing |
| `CLARITY_VISION_MODEL`, `CLARITY_RESOLVER_MODEL` | No | AI Gateway model ids (`provider/model`, from https://ai-gateway.vercel.sh/v1/models) for reading receipts and resolving line items; defaults in `modules/finance/core/receipts/model-ids.ts` |
...
| `PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ENV` | Private deployment | Plaid API credentials for bank sync; `PLAID_ENV` is `sandbox` or `production` and `PLAID_SECRET` must be the secret for that environment. Unset → the Banks page is read-only and `/api/plaid/*` answers 503. In the sandbox the page can connect Plaid's test institution without Link |
| `PLAID_TOKEN_KEY` | Private deployment | 32 bytes as 64 hex chars (`openssl rand -hex 32`); encrypts Plaid access tokens at rest (AES-256-GCM). Losing it means reconnecting every bank |
