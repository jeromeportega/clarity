...
| `AI_GATEWAY_API_KEY` | Local / CI only | Vercel AI Gateway key for live vision + SKU resolution off Vercel (scripts, `npm run vision:eval`, another host). On Vercel nothing is needed: the deployment authenticates to the gateway with its own OIDC token. Without any credential the upload route falls back to recorded fixtures — fine for tests, useless for real receipts |
| `RECEIPT_AI` | No | `live` or `recorded` to force the vision/resolver choice instead of inferring it from the credentials above |
